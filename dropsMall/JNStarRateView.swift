...
class JNStarRateView: UIView {
    ...
    var usePanAnimation:Bool = true//滑动评分是否使用动画，默认为false
    var allowUnderCompleteStar:Bool = false{//是否允许非整星评分，默认为true
        willSet{
    ...
    fileprivate var _allowUserPan:Bool = true//默认不支持滑动评分
    ...
    required init?(coder aDecoder: NSCoder) {
     ...
     func createStarView()->(){
        starBackgroundView = starViewWithImageName("evaluationOfEmptyStar")
        starForegroundView = starViewWithImageName("realStarEvaluation")
...
//        let width = self.frame.size.width / CGFloat(count)
        for index in 0...count {
            let imageView = UIImageView.init(frame: CGRect(x:CGFloat(index) * self.mj_h,y: 0,width:self.mj_h,height:self.mj_h))
